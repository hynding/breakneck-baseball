//! Team palette, rig construction/spawning, per-frame recolouring, and the
//! identity sync that keeps every seated rig's [`PlayerIdentity`] matching
//! the live game.

use bevy::prelude::*;
use bevy_rapier3d::prelude::*;

use crate::game::animation::{LimbKind, MoveIntent, RigBaseY, RigLimb, bat_idle_rotation};
...
/// Tag on every recolourable rig child. `pub(super)` — `recolor_teams`
/// (`pub(super)`, called from `PlayerPlugin::build` in `mod.rs`) queries it
/// by reference, and edition 2024's `private_interfaces` lint requires the
/// query's component type to be at least as visible as the system function.
#[derive(Component)]
pub(super) struct RigPart {
    unit: RigUnit,
    part: PartKind,
}
