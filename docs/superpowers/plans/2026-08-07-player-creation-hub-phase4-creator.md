...
- `enter_creator`: on MainMenu, `keyboard.just_pressed(KeyCode::KeyC)` → `next_state.set(GameState::Creator)`; gate with `.run_if(crate::game::settings::settings_closed)` (the `cycle_options`/`menu_select` precedent) so C can't fire behind an open Settings screen. (KeyC is also `camera::toggle_camera_mode` — Playing-only, no conflict; acknowledge in a comment.)
- `exit_creator`: in Creator, Esc → `next_state.set(GameState::MainMenu)`. The revert itself is owned by an `OnExit(GameState::Creator)` system (Task 2) — decoupled from the keypress so it fires on EVERY exit path.
- `OnEnter(Creator)`: spawn (all with a `CreatorStage` marker for teardown): a 12×12 ground plane (reuse the field's grass color or a neutral `Color::srgb(0.25, 0.45, 0.25)`), three point/directional lights (key/fill/rim — intensities to taste), a `Camera3d` at roughly `(1.8, 1.6, 2.6)` looking at `(0, 1.0, 0)`, and the preview rig: construct the `RigModel` the same way `spawn_players` does (match on `theme.player_model`; extract that construction into a small `pub(crate) fn build_rig_model(...)` in player.rs rather than duplicating it), call `spawn_rig` at `Vec3::new(0.0, 0.6, 0.0)` facing the camera, insert `PreviewRig` + `Batter` + `PlayerIdentity { team: cs.team, index: cs.index }`, and `attach_jerseys` (create `JerseyAssets` via `jersey::make_assets` if the resource is absent — game start normally makes it). The `Batter` marker is required: `wire_rigs`'s `Has<Batter>` check otherwise hides the bat submesh, breaking the Animations tab's stance/fidget/celebration previews (all imply a held bat). Safe: `batter_stance`/`trigger_swing`/`celebrate_home_run` stay `run_if(in_state(Playing))` and never see the preview rig.
- A Creator-scoped retint system: on selection change, reassign the preview rig's `GltfJerseyMesh`-tagged jersey/cap meshes to `team_mats.jersey(cs.team)`/`team_mats.cap(cs.team)`. Without it the panel's team toggle silently fails to retint the uniform — `recolor_gltf`/`recolor_teams` key off `ScoreBoard` (not `PlayerIdentity`) and stay Playing-gated; only gear props would follow the team (via `dress_rigs`' direct `team_mats.cap(team)` call).
...
`cargo test` AND — **the FULL suite with the debug feature, not just the new test**: `cargo test --features debug --no-fail-fast`. This is the first time the whole e2e suite runs with `EguiPlugin` compiled in (CI's debug job only `cargo check`s); catch any headless-egui surprise NOW, before Tasks 2–4 build on it. (A spot-check of `e2e_pause_subs --features debug` passed at plan time — de-risked, not proven.) Then `cargo check` (no features) + wasm check; clippy both feature sets; fmt.
...
- Every edit mutates `selected_def(&mut cs.working, ...)` — nothing else. A change-application system (separate from the panel, `Changed<CreatorState>`-driven or a dirty flag) rebuilds: `*roster_defs = RosterDefs(cs.working.clone()); let (rosters, id) = preview_rosters_and_identity(...); *live_rosters = rosters;` and re-inserts `id` on the preview rig.
- Insert `PlayerIdentity` unconditionally on every apply — do NOT gate it on team/index differing from what's already stamped. Insertion (not value-equality) is the change signal `dress_rigs`/`sync_identities` react to (see `player::sync_identities`'s "Inserting is the change signal" doc comment) — a same-value re-insert on a pure appearance edit IS the retrigger mechanism, not waste. Put that sentence in a code comment at the call site.
- **REQUIRED sub-step — fix the watcher-clobber bug**: mutating `RosterDefs` live diverges it from disk, and `dev_watch::watch_players_file` compares freshly-read disk text against the LIVE `defs.0` — so on its next 1 s poll it treats the unchanged file as "new" and silently reverts the panel's edit. Fix `appearance.rs::dev_watch::watch_players_file` to hold `Local<Option<String>>` of the last-seen DISK text and short-circuit (return) when disk text is unchanged, before ever calling `apply_reload`. Disk really changing still wins (spec §6's round-trip stays intact); the watcher just stops re-detecting its own stale baseline. `apply_reload`'s pure-fn unit tests are unaffected; add one watcher-behavior note in its doc comment.
- **Revert button**: `cs.working = cs.snapshot.clone()` + reapply. The same revert-and-reapply also runs from an `OnExit(GameState::Creator)` system, so every exit path (Esc, future menu buttons) restores unsaved edits. **Save/Randomize/Portraits buttons** land in Tasks 3–4; render them disabled with a tooltip for now is fine, or omit.
...
**Documented deviation from spec §6**: the spec's literal wording is "boots windowless… render-to-texture + readback"; this harness instead does a **windowed** automated run using `Screenshot::primary_window()` (verified Bevy 0.15 API: `commands.spawn(Screenshot::primary_window()).observe(save_to_disk(path))`). True windowless capture needs `Screenshot::image()` + a manual off-screen camera target — materially more plumbing for the identical AI-facing outcome (one command, PNGs, unattended). Deliberate simplification; the window briefly appearing on the dev machine is accepted.
