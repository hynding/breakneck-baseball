...
fn run_cell(mode: MatrixMode, style: BattingStyle, plays: u32) {
    ...
    assert!(
        ...
        "{mode:?}/{style:?}: only {results}/{plays} plays resolved"
    );
    if !matches!(mode, MatrixMode::CpuVsCpu) {
        assert!(
            saw_swing,
            "{mode:?}/{style:?}: the scripted batter never got a swing judged \
